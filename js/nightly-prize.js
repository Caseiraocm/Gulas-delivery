...
(()=>{
  ...
  const winnerHtml=prize=>prize?.is_winner?`<div class="nightPrizeWin"><h3>VOCÊ GANHOU!</h3><strong class="nightPrizeName">${esc(prize.prize_name||'Brinde do Gullas')}</strong><p>${esc(prize.winner_message||'Você ganhou um brinde do Gullas Hamburgueria!')}</p><span class="nightPrizeCode">${esc(prize.winner_code||'')}</span><div class="nightPrizeHint">Tire um print desta tela e envie para o Gullas. Guarde também o código acima.</div></div>`:'';
  sendOrder=async function(){
    ...
    try{
      ...
      btn.disabled=true;btn.textContent='ENVIANDO...';sessionStorage.setItem('gulas_pending_request',JSON.stringify(payload));
      const j=await api('create-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      sessionStorage.removeItem('gulas_pending_request');
      ...
      localStorage.setItem('gulas_last_tracking',JSON.stringify({tracking_code:j.tracking_code,phone:customer.phone}));
      cart=[];updateCartBar();
