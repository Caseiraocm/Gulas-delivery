...
(()=>{
  const polishAdminLabels=()=>{
    ...
    if(!head||!/^Central Gullas$/i.test(head.textContent.trim()))return;
    ...
    if(nav){nav.setAttribute('aria-label','Áreas da Central Gullas');nav.querySelectorAll('button').forEach(button=>button.setAttribute('aria-pressed',String(button.classList.contains('on'))));}
  };
